...
#[test]
fn test_benchmark_sweep_window_inside_vs_outside() {
    use std::time::Instant;

    let capacity = 1u64 << 18; // 2^18 = 262,144 blocks
    let num_ops = 200_000usize;
    let warmup_ops = 20_000usize;
    let num_trials = 10usize;

    let mut outside_latencies = Vec::with_capacity(num_trials);
    let mut inside_latencies = Vec::with_capacity(num_trials);

    for trial in 0..num_trials {
        let seed = 42u64 + (trial as u64) * 1000;
        let mut rng = StdRng::seed_from_u64(seed);

        let keys: Vec<[u8; 16]> = (0..(warmup_ops + num_ops))
            .map(|_| {
                let mut k = [0u8; 16];
                rng.fill(&mut k);
                k
        ...
        // 1. Outside sweep window (Standard Normal Operation)
        {
            let mut hist = ObliviousHistogram::<16, 16, 20, 64, u64>::new(capacity, &mut rng);
            for k in &keys[..warmup_ops] {
                hist.append(k, 1);
            }
            hist.flush();
            ...
            outside_latencies.push(us_per_op);
        }

        // 2. Inside sweep window (Active migration: 1 extra bucket per eviction)
        {
            let mut hist = ObliviousHistogram::<16, 16, 20, 64, u64>::new(capacity, &mut rng);
            hist.set_sweep_window_active(true); // forces reading extra sibling bucket on every eviction
            ...
            let elapsed = start.elapsed();
            let us_per_op = elapsed.as_micros() as f64 / num_ops as f64;
            inside_latencies.push(us_per_op);
        }
    }

    outside_latencies.sort_by(|a, b| a.partial_cmp(b).unwrap());
    inside_latencies.sort_by(|a, b| a.partial_cmp(b).unwrap());

    let med_outside = outside_latencies[num_trials / 2];
    let mean_outside = outside_latencies.iter().sum::<f64>() / num_trials as f64;
    let std_outside = (outside_latencies.iter().map(|x| (x - mean_outside).powi(2)).sum::<f64>() / num_trials as f64).sqrt();

    let med_inside = inside_latencies[num_trials / 2];
    let mean_inside = inside_latencies.iter().sum::<f64>() / num_trials as f64;
    let std_inside = (inside_latencies.iter().map(|x| (x - mean_inside).powi(2)).sum::<f64>() / num_trials as f64).sqrt();

    let diff = med_inside - med_outside;
    let pct = (diff / med_outside) * 100.0;

    println!("\n========================================================================");
    println!("  SWEEP WINDOW BENCHMARK RESULTS (N=2^18, Z=16, A=20, 10 Trials):");
    println!("  -> Outside Sweep Window : {:.4} ± {:.4} us/op", med_outside, std_outside);
    println!("  -> Inside Sweep Window  : {:.4} ± {:.4} us/op", med_inside, std_inside);
    println!("  -> Sweep Window Delta   : +{:.4} us/op (+{:.2}%)", diff, pct);
    println!("========================================================================\n");
}
