...
using ParMapType = ODSL::ParOMap<ParOMapK, ParOMapV, uint32_t>;
...
void ParOMapBinding::InsertBatch(uint32_t batchSize, const ParOMapK* keys,
...
void ParOMapBinding::FindBatch(uint32_t batchSize, const ParOMapK* keys,
...
void ParOMapBinding::FindBatchDeferMaintain(uint32_t batchSize,
                                            const ParOMapK* keys,
                                            ParOMapV* vals, bool* existFlags) {
  Assert(!initializer, "Find during initialization");
  std::vector<uint8_t> resFlags =
...
void ParOMapBinding::EraseBatch(uint32_t batchSize, const ParOMapK* keys,
                                bool* existFlags) {
