...
consteval EventId CE_GetFuncEventId(
    const char* funcName = SOURCE_LOCATION::current().function_name()) {
#define F(name) \
  if (CE_StringEquals(funcName, #name)) return EventId::name;
#include "./tracer_events.hxx"
...
constexpr uint64_t TOTAL_TRACKERS =
    static_cast<std::underlying_type_t<EventId> >(EventId::LAST_EVENT);
...
#define PROFILE_F()                                          \
  typename Profiler::BlockProfiler_t _CONCAT_LINE(profiler)( \
      CE_GetFuncEventId(__FUNCTION__));
#define PROFILE_B(name)                                              \
  typename Profiler::BlockProfiler_t _CONCAT_LINE(_profiler_##name)( \
      EventId::name);
...
#define PROFILE_V(varName, val) \
  Profiler::g_Tracker.TrackValue(EventId::varName, val)
#else
#define PROFILE_V(varName, val)
#endif
...
#define TRACE_F()                                           \
  typename TimeTracker::BlockTracer_t _CONCAT_LINE(tracer)( \
      CE_GetFuncEventId(__FUNCTION__));
#define TRACE_B(name)                                               \
  typename TimeTracker::BlockTracer_t _CONCAT_LINE(_tracer_##name)( \
      EventId::name);
