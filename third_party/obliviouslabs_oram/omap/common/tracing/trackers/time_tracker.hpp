...
struct TimeTracker {
  using BlockTracer_t =
      BlockTracer<TimeTracker, std::chrono::system_clock::time_point>;
  ...
  explicit TimeTracker() : stack{0} {
    std::cout << "Building tracker with " << TOTAL_TRACKERS << " total tracers."
              << std::endl;
  ...
  INLINE void Measure(const EventId eventId,
