...
#define aex_ctx_t EVP_CIPHER_CTX
...
void secure_hash_with_salt(const T& data, const uint8_t (&salt)[16],
  ...
  // Hash the data
  SHA256_Update(&ctx, &data, sizeof(T));
  ...
  memcpy(res, hash,
         resSize < SHA256_DIGEST_LENGTH ? resSize : SHA256_DIGEST_LENGTH);
}
