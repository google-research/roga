...
INLINE void CMOV8_internal(const uint64_t cond, void* guy1_ptr,
                           const void* guy2_ptr) {
  uint64_t guy1, guy2;
  memcpy(&guy1, guy1_ptr, 8);
  memcpy(&guy2, guy2_ptr, 8);
  asm volatile(
      "test %[mcond], %[mcond]\n\t"
      "cmovnz %[i2], %[i1]\n\t"
      : [i1] "=r"(guy1)
      ...
      :);
  memcpy(guy1_ptr, &guy1, 8);
}

INLINE void CMOV4_internal(const uint64_t cond, void* guy1_ptr,
                           const void* guy2_ptr) {
  uint32_t guy1, guy2;
  memcpy(&guy1, guy1_ptr, 4);
  memcpy(&guy2, guy2_ptr, 4);
  asm volatile(
      "test %[mcond], %[mcond]\n\t"
      "cmovnz %[i2], %[i1]\n\t"
      : [i1] "=r"(guy1)
...
INLINE void CXCHG_internal(const bool cond, void* vec1, void* vec2) {
  ...
  if constexpr (sz % 32 >= 16) {
    // byte_offset = (sz/32)*32 = byte position after the 32-byte chunks
    constexpr uint64_t byte_offset = (sz / 32) * 32;
#if defined(__AVX512VL__)
    __m128d vec1_temp, vec2_temp;
    std::memcpy(&vec1_temp, (char*)vec1 + byte_offset, 16);
    std::memcpy(&vec2_temp, (char*)vec2 + byte_offset, 16);
    ...
    std::memcpy((char*)vec1 + byte_offset, &vec1_after_swap, 16);
    std::memcpy((char*)vec2 + byte_offset, &vec2_after_swap, 16);
    ...
    std::memcpy((char*)vec1 + byte_offset, &vec1_temp, 16);
    std::memcpy((char*)vec2 + byte_offset, &vec2_temp, 16);
#else
    CXCHG_internal<8>(cond, (char*)vec1 + byte_offset,
                      (char*)vec2 + byte_offset);
    CXCHG_internal<8>(cond, (char*)vec1 + byte_offset + 8,
                      (char*)vec2 + byte_offset + 8);
#endif
  }

  if constexpr (sz % 16 >= 8) {
    constexpr uint64_t byte_offset = (sz / 16) * 16;
    uint64_t v1, v2;
    memcpy(&v1, (char*)vec1 + byte_offset, 8);
    memcpy(&v2, (char*)vec2 + byte_offset, 8);
    CXCHG8(cond, v1, v2);
    memcpy((char*)vec1 + byte_offset, &v1, 8);
    memcpy((char*)vec2 + byte_offset, &v2, 8);
  }
  if constexpr (sz % 8 >= 4) {
    constexpr uint64_t byte_offset = (sz / 8) * 8;
    uint32_t v1, v2;
    memcpy(&v1, (char*)vec1 + byte_offset, 4);
    memcpy(&v2, (char*)vec2 + byte_offset, 4);
    CXCHG4(cond, v1, v2);
    memcpy((char*)vec1 + byte_offset, &v1, 4);
    memcpy((char*)vec2 + byte_offset, &v2, 4);
  }
  if constexpr (sz % 4 >= 2) {
    constexpr uint64_t byte_offset = (sz / 4) * 4;
    uint16_t v1, v2;
    memcpy(&v1, (char*)vec1 + byte_offset, 2);
    memcpy(&v2, (char*)vec2 + byte_offset, 2);
    CXCHG2(cond, v1, v2);
    memcpy((char*)vec1 + byte_offset, &v1, 2);
    memcpy((char*)vec2 + byte_offset, &v2, 2);
  }
  if constexpr (sz % 2 >= 1) {
    constexpr uint64_t byte_offset = (sz / 2) * 2;
    uint8_t v1, v2;
    memcpy(&v1, (char*)vec1 + byte_offset, 1);
    memcpy(&v2, (char*)vec2 + byte_offset, 1);
    CXCHG1(cond, v1, v2);
    memcpy((char*)vec1 + byte_offset, &v1, 1);
    memcpy((char*)vec2 + byte_offset, &v2, 1);
  }
}
...
INLINE void CMOV_internal(const bool cond, void* dest, const void* src) {
  ...
  if constexpr (sz % 32 >= 16) {
    constexpr uint64_t byte_offset = (sz / 32) * 32;
    const char* src8 = (const char*)src + byte_offset;
    char* dest8 = (char*)dest + byte_offset;
#if defined(__SSE2__)
    const __m128i mask =
    ...
    CMOV_internal<8>(cond, dest8 + 8, src8 + 8);
#endif
  }

  if constexpr (sz % 16 >= 8) {
    constexpr uint64_t byte_offset = (sz / 16) * 16;
    CMOV8_internal(cond, (char*)dest + byte_offset,
                   (const char*)src + byte_offset);
  }
  if constexpr (sz % 8 >= 4) {
    constexpr uint64_t byte_offset = (sz / 8) * 8;
    CMOV4_internal(cond, (char*)dest + byte_offset,
                   (const char*)src + byte_offset);
  }
  if constexpr (sz % 4 >= 2) {
    constexpr uint64_t byte_offset = (sz / 4) * 4;
    uint16_t v1, v2;
    memcpy(&v1, (char*)dest + byte_offset, 2);
    memcpy(&v2, (const char*)src + byte_offset, 2);
    uint32_t r1 = v1;
    uint32_t r2 = v2;
    CMOV4_internal(cond, &r1, &r2);
    v1 = r1 & 0xffff;
    memcpy((char*)dest + byte_offset, &v1, 2);
  }
  if constexpr (sz % 2 >= 1) {
    constexpr uint64_t byte_offset = (sz / 2) * 2;
    uint8_t v1, v2;
    memcpy(&v1, (char*)dest + byte_offset, 1);
    memcpy(&v2, (const char*)src + byte_offset, 1);
    uint32_t r1 = v1;
    uint32_t r2 = v2;
    CMOV4_internal(cond, &r1, &r2);
    v1 = r1 & 0xff;
    memcpy((char*)dest + byte_offset, &v1, 1);
  }
}

template <typename T>
INLINE void obliSwap(const bool mov, T& guy1, T& guy2) {
  // Use byte pointers to avoid alignment issues with __m512i
  char* curr1 = (char*)&guy1;
  char* curr2 = (char*)&guy2;
...
INLINE bool obliMove(const bool mov, T& dest, const T& src) {
  // Use byte pointers to avoid alignment issues with __m512i
  char* curr1 = (char*)&dest;
  const char* curr2 = (const char*)&src;
  for (uint64_t i = 0; i < sizeof(T) / 64; ++i) {
