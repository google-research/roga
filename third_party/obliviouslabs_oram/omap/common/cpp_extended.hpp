...
inline void Log_Recursive(const char* file, int line, std::ostringstream& msg) {
  ...
  if constexpr (logTime) {
    std::cerr << "[" << now_c.time_since_epoch().count() / 1000000000 << "."
              << ((now_c.time_since_epoch().count() % 1000000000) / 1000)
              << "]";
  }
...
// c++20 is not compatible with c++14 libraries for the << for unsigned int's,
// so we overload with to_string:
//
...
void Log_Recursive(const char* file, int line, std::ostringstream& msg, T value,
                   const Args&... args) {
  Log_One(msg, value);
...
#define static_false(msg) \
  []<bool flag = false>() { static_assert(flag, msg); }();

template <typename F>
struct _defer_class {
  explicit _defer_class(F&& f) : _f(std::forward<F>(f)) {}
  ~_defer_class() { _f(); }
...
#define Assert(expr, ...)                               \
  if (expr) [[likely]] {                                \
    ...
    X_LOG("Assertion violated: {", #expr,               \
          "}" __VA_OPT__(, NAMED_VALUES(__VA_ARGS__))); \
    X_FAIL();                                           \
  }
#define X_LOG(...) LogWrapper(__FILE__, __LINE__, __VA_ARGS__, "\n")
#define X_LOG_SIMPLE(...) \
  LogWrapper<false, false>(__FILE__, __LINE__, __VA_ARGS__)
#else
#define Assert(expr, ...)                          \
  if (expr) [[likely]] {                           \
  } else {                                         \
    printf("Assertion violated: { %s }\n", #expr); \
    X_FAIL();                                      \
  }
#define X_LOG(...)
#define X_LOG_SIMPLE(...)
