 ...
 private:
  ...
  void heapify_up(uint32_t i) {
    while (i != 0) {
      ...
      ORAM::obliSwap(heap_i, heap_parent, heap_parent > heap_i);
      heap[i] = heap_i;
  ...
  void heapify_down(uint32_t i) {
    ...
    // smallest = ORAM::oblivious_select(smallest, left, left < size() &&
    // heap[left] < heap_i);
    T heap_smallest = heap[smallest];
    ORAM::CMOV(right < size() && heap[right] < heap_smallest, smallest, right);
    // smallest = ORAM::oblivious_select(smallest, right, right < size() &&
    // heap[right] < heap_smallest);
