...
class OTwoTierHash : public OHashBase<KeyType, BlockSize> {
 ...
 public:
  static std::tuple<KeyType, double> compute_epsilon_inv(
      Block<KeyType, BlockSize>* data, const KeyType n,
    ...
    while (bin_size < n) {
      ...
      Timer t;
      oht.build(data);
      for (KeyType i = 0; i < n; i++) oht[dist(gen)];
      oht.extract();
      ...
      std::cout << _ << "epsilon_inv: " << epsilon_inv
                << ", bin_size: " << bin_size << ", time: " << cur_time
                << ", memory usage: " << getMemoryUsage() << " KB" << std::endl;
    }
  ...
  OTwoTierHash(const KeyType n,
      ...
      : n(n),
        ...
        bin_num(n > bin_size ? KeyType(2) * n / bin_size
                             : KeyType(1)),  // public
        prf(bin_num),
        bin_loads(bin_num),
        major_bins(bin_num, ObliviousBin<KeyType, BlockSize>(
                                bin_num == 1 ? n : bin_size / 2, n / bin_num,
                                delta_inv_log2)),
        overflow_bin(bin_num > 1 ? n / epsilon_inv : 0, bin_num > 1 ? n : 0,
                     delta_inv_log2),
  ...
  virtual void build(Block<KeyType, BlockSize>* data) {
    ...
    std::generate(shuffle_key.begin(), shuffle_key.end(), [this]() {
      static std::uniform_int_distribution<size_t> dis(
          0, std::numeric_limits<size_t>::max());
      return dis(this->gen);
    });
    auto shuffle_key_st = shuffle_key.data();
    osorter(data, n, [data, shuffle_key_st](const auto& a, const auto& b) {
    ...
    std::vector<uint32_t> secrete_loads =
        sample_secrete_loads(n - n / epsilon_inv);
    ...
    std::for_each(
        ...
        [&](KeyType i) {
          ...
          for (KeyType j = tmp; j < bin_st + (KeyType)bin_loads[i]; j++) {
            obliSwap(buffer[j], overflow_data[overflow_st + j - tmp],
                     j >= bin_st + secrete_loads[i]);
          }
        });
    // build major bins, w/ overwhleming probability, the major bins are less
    // than half
    std::for_each(
        std::execution::par_unseq, indices.begin(), indices.end(),
        [&](KeyType i) { major_bins[i].build(buffer.data() + i * bin_size); });
    ...
    ocompact_by_half(overflow_data.begin(), flags.begin(), overflow_data.size(),
                     OCOMPACT_Z);
  ...
  virtual std::vector<Block<KeyType, BlockSize>>& extract() {
    ...
    for (auto& e : data)
      tmp.emplace_back(oblivious_select(prf(e.id), bin_num, e.dummy()), e);
    // std::cout << "Time for overflow pile: " << t.get_interval_time() <<
    // std::endl;
    KeyType group_size = bin_size / epsilon_inv;
    for (KeyType i = 0; i < bin_num; i++)
      for (KeyType j = 0; j < group_size; j++) tmp.emplace_back(i, dummy_block);
    ...
    extracted_data.resize(
        std::accumulate(bin_loads.begin(), bin_loads.end(), 0));

    std::for_each(
        std::execution::par_unseq, major_bins.begin(), major_bins.end(),
        [&](auto& bin) {
          // auto bin_data = bin.data();
          auto bin_data = bin.extract();
          auto tmp_size = bin_data.size();
          bin_data.resize(tmp_size + group_size);
          // Assuming tmp and group_size are accessible here and bin_size is
          // defined
          for (KeyType j = 0; j < group_size; j++)
            bin_data[j + tmp_size] =
                tmp[(&bin - &major_bins[0]) * group_size + j].second;
          ...
          std::copy(bin_data.begin(),
                    bin_data.begin() + bin_loads[&bin - &major_bins[0]],
                    extracted_data.begin() +
                        std::accumulate(
                            bin_loads.begin(),
                            bin_loads.begin() + (&bin - &major_bins[0]), 0));
        });
    return extracted_data;
  }
  ...
  bool operator==(const OTwoTierHash& other) const {
    ...
    if (bin_num != other.bin_num) return false;
    if (epsilon_inv != other.epsilon_inv) return false;
    if (major_bins.size() != other.major_bins.size()) return false;
    for (size_t i = 0; i < major_bins.size(); i++)
      if (major_bins[i] != other.major_bins[i]) return false;
    return true;
  }

  virtual OHashBase<KeyType, BlockSize>* clone() {
    auto ret = new OTwoTierHash<KeyType, BlockSize>(
        this->n, this->delta_inv_log2, this->epsilon_inv);
