...
static void bitonic_merge(DataIt data, uint64_t low, uint64_t cnt, bool dir,
                          Comparator cmp) {
  if (cnt <= 1) return;
  uint64_t k = 1;
  while (k < cnt) k <<= 1;
  ...
  for (uint64_t i = low; i < low + cnt - k; i++)
    obliSwap(data[i], data[i + k], dir == !cmp(data[i], data[i + k]));
  if (cnt >= 64)
    tbb::parallel_invoke(
...
template <std::random_access_iterator DataIt,
          typename Comparator =
              std::less<typename std::iterator_traits<DataIt>::value_type>>
void bitonic_sort(DataIt data, uint64_t low, uint64_t cnt, bool dir,
