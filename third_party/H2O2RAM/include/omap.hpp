...
class ObliviousMap {
 ...
 public:
  // implement an oblivious & cryptographically secure hash alg is orthogonal to
  // our task
...
class ObliviousMap<uint16_t, ValueType> {
 ...
 public:
  ...
  inline void erase(const uint16_t key) { oram.erase(key); }

  inline ValueType& operator[](const uint16_t key) { return oram[key]; }
};
...
class ObliviousMap<uint32_t, ValueType> {
 ...
 public:
  ...
  inline void erase(const uint32_t key) { oram.erase(key); }

  inline ValueType& operator[](const uint32_t key) { return oram[key]; }
};
...
class ObliviousMap<uint64_t, ValueType> {
 ...
 public:
  ...
  inline void erase(const uint64_t key) { oram.erase(key); }

  inline ValueType& operator[](const uint64_t key) { return oram[key]; }
};
}  // namespace ORAM