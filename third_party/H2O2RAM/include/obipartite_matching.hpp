...
struct BiEdge {
  ...
  EdgeFlag flag;
  BiEdge(T u = 0, T v = 0) : u(u), v(v), ctr(0), dir(1), flag(NA) {}
};
// #pragma pack(pop)
...
template <std::unsigned_integral T>
std::vector<T> omatcher(std::vector<BiEdge<T>>& edges, const T left_cnt,
                        const T k) {
  assert(left_cnt * k == edges.size());
  BiEdge<T>** edges_by_bucket = new BiEdge<T>*[k];
  for (T i = 0; i < k; i++) edges_by_bucket[i] = new BiEdge<T>[left_cnt];
  ...
  for (int z = 0; z < UB; z++) {
    ...
    std::for_each(
        std::execution::par_unseq, indices.begin(), indices.end(), [&](auto _) {
          const T i = _ * k;
          auto& e0 = edges[i];
          matchL[e0.u] = e0.v;
          e0.ctr += e0.dir;
          ...
          osorter(
              edges.data() + i, k,
              [](const BiEdge<T>& a, const BiEdge<T>& b) { return a.v < b.v; });
          for (T j = 0; j < k; j++) edges_by_bucket[j][i / k] = edges[i + j];
        });
    std::for_each(std::execution::par_unseq, edges_by_bucket,
                  edges_by_bucket + k, [left_cnt](auto& bucket) {
    ...
    std::for_each(std::execution::par_unseq, indices.begin(),
                  indices.begin() + k, [&](auto i) {
                    ...
                    for (T j = 0; j < left_cnt; j++) {
                      ...
                      CMOV(!first_edge & (e->dir == 0), e->flag,
                           REVERSE_IF_POSSIBLE_BK);
  ...
  for (T i = 0; i < k; i++) delete[] edges_by_bucket[i];
  delete[] edges_by_bucket;
  return matchL;
}
