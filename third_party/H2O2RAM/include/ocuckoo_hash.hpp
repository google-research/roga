...
class OCuckooHash : public OHashBase<KeyType, BlockSize> {
 ...
 public:
  OCuckooHash(KeyType n, KeyType delta_inv_log2 = DELTA_INV_LOG2)
  ...
  virtual void build(Block<KeyType, BlockSize>* data) {
    ...
    for (uint32_t i = 0; i < prf_cnt; i++) prfs.emplace_back(bucket_size);
    ...
    for (KeyType i = 0; i < n; i++) {
      for (KeyType j = 0; j < prf_cnt; j++) {
        KeyType dest_id = prfs[j](data[i].dummy() ? --dummy_ctr : data[i].id) +
                          j * bucket_size;
    ...
    tmp.reserve(n * 3ll);
    for (KeyType i = 0; i < n; i++) tmp.emplace_back(matches[i], data[i]);
    for (KeyType i = 0; i < 2 * n; i++) tmp.emplace_back(i, dummy_block);
    ...
    entries.clear();
    for (KeyType i = 0; i < 2 * n; i++) entries.emplace_back(tmp[i].second);
  }
  ...
  virtual Block<KeyType, BlockSize> operator[](KeyType i) {
    ...
    for (uint32_t j = 0; j < prf_cnt; j++) {
      ...
      //                         _.id | (KeyType(1) << (8 * sizeof(KeyType) -
      //                         1)),
      //                         _.id == i);
    }
    return ret;
  ...
  virtual std::vector<Block<KeyType, BlockSize>>& extract() {
    ...
    for (KeyType i = 0; i < entries.size(); i++) flags[i] = !entries[i].dummy();
    // ocompact
    ocompact_by_half(entries.begin(), flags.begin(), entries.size(),
                     OCOMPACT_Z);
  ...
  virtual OHashBase<KeyType, BlockSize>* clone() {
    auto ret =
        new OCuckooHash<KeyType, BlockSize>(this->n, this->delta_inv_log2);
