...
static long double bin_load(size_t n, size_t m, size_t k) {
  if (n == k) return std::pow(1.0 / m, k);
  if (k == 0) return std::pow(1 - 1.0 / m, n);
  long double ret =
      log2_nCr(n, k) + (n - k) * std::log2(m - 1) - n * std::log2(m);
...
class OHashBucket : public OHashBase<KeyType, BlockSize> {
 ...
 public:
  static std::tuple<double, KeyType, KeyType> compute_appropriate_bucket_num(
      Block<KeyType, BlockSize>* data, KeyType n, KeyType op_num,
    ...
    auto compute_time = [&](KeyType bucket_num) {
      ...
      for (int _ = 0; _ < T; _++) {
        ...
        Timer t;
        obucket.build(data);
        for (KeyType i = 0; i < op_num; i++) obucket[dis(gen)];
        obucket.extract();
    ...
    // for (KeyType bucket_num = bucket_num_l; bucket_num < bucket_num_r;
    // bucket_num++)
    // {
    //     if (bucket_num <= 2)
    //         continue;
    //     KeyType bucket_size = compute_bucket_size(n, bucket_num,
    //     delta_inv_log2); if (bucket_size * bucket_num < n)
    //         continue;
  ...
  OHashBucket(KeyType n, KeyType bucket_num, KeyType bucket_size)
      : n(n),
  ...
  virtual void build(Block<KeyType, BlockSize>* data) {
    ...
    tmp.reserve(n + d_idx);
    for (KeyType i = 0; i < n; ++i) tmp.emplace_back(prf(data[i].id), data[i]);
    ...
    for (KeyType i = 0; i < d_idx; i++) this->entries[i] = tmp[i].second;
  }
  ...
  virtual std::vector<Block<KeyType, BlockSize>>& data() { return entries; }
  ...
  virtual OHashBase<KeyType, BlockSize>* clone() {
    auto ret = new OHashBucket<KeyType, BlockSize>(this->n, this->bucket_num,
                                                   this->bucket_size);
    ret->prf = this->prf;
