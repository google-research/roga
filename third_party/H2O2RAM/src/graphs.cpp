...
ORAM::ObliviousRAM<uint32_t, Edge> init_edge(
    std::vector<Edge>& edges_)  //  init_edge(int m)
{
...
void add_edge(
    std::vector<Edge>& edges,  // ORAM::ObliviousRAM<uint32_t, Edge> &edges,
    ORAM::ObliviousRAM<uint32_t, int>& head, int source, int to, int weight,
    int idx) {
...
void add_edge(ORAM::ObliviousRAM<uint32_t, Edge>& edges,
              ORAM::ObliviousRAM<uint32_t, int>& head, int source, int to,
              int idx) {
  // assume edges has been initialized
  assert(edges.size() > idx);
  auto& e = edges[idx];
...
ORAM::ObliviousRAM<uint32_t, int> shortest_path(
    const ORAM::ObliviousRAM<uint32_t, Edge>& edges,
    const ORAM::ObliviousRAM<uint32_t, int>& head, int source) {
  int n = head.size();
  int m = edges.size();
