...
#[cfg_attr(feature = "serde", serde(bound(serialize = "W: Serialize, W::Sampler: Serialize")))]
#[cfg_attr(
    feature = "serde",
...
impl<W: AliasableWeight> WeightedAliasIndex<W> {
    ...
    pub fn new(weights: Vec<W>) -> Result<Self, Error> {
        ...
        let max_weight_size = W::try_from_u32_lossy(n).map(|n| W::MAX / n).unwrap_or(W::ZERO);
        if !weights.iter().all(|&w| W::ZERO <= w && w <= max_weight_size) {
        ...
        let weight_sum = if weight_sum > W::MAX { W::MAX } else { weight_sum };
...
mod test {
    ...
    fn test_weighted_index_f32() {
        ...
        assert_eq!(WeightedAliasIndex::new(vec![f32::INFINITY]).unwrap_err(), Error::InvalidWeight);
        assert_eq!(WeightedAliasIndex::new(vec![-0_f32]).unwrap_err(), Error::InsufficientNonZero);
        assert_eq!(WeightedAliasIndex::new(vec![-1_f32]).unwrap_err(), Error::InvalidWeight);
        assert_eq!(
    ...
    fn test_weighted_index_i128() {
        ...
        assert_eq!(WeightedAliasIndex::new(vec![-1_i128]).unwrap_err(), Error::InvalidWeight);
    ...
    fn test_weighted_index_i8() {
        ...
        // Signed integer special cases
        assert_eq!(WeightedAliasIndex::new(vec![-1_i8]).unwrap_err(), Error::InvalidWeight);
        ...
        WeightedAliasIndex<W>: fmt::Debug,
    {
        ...
        assert_eq!(WeightedAliasIndex::<W>::new(vec![]).unwrap_err(), Error::InvalidInput);
        assert_eq!(WeightedAliasIndex::new(vec![W::ZERO]).unwrap_err(), Error::InsufficientNonZero);
        assert_eq!(
