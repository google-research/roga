    ...
    WeightedTreeIndex<W>
{
        ...
        I::Item: SampleBorrow<W>,
    {
        ...
        for i in (1..n).rev() {
            ...
            let parent = (i - 1) / 2;
            subtotals[parent].checked_add_assign(&w).map_err(|()| Error::Overflow)?;
        }
        Ok(Self { subtotals })
    }
    ...
    pub fn update(&mut self, mut index: usize, weight: W) -> Result<(), Error> {
        ...
        if weight > old_weight {
            ...
            self.subtotals[index].checked_add_assign(&difference).unwrap();
            while index != 0 {
                index = (index - 1) / 2;
                self.subtotals[index].checked_add_assign(&difference).unwrap();
            }
...
mod test {
    ...
    fn test_no_item_error() {
        ...
        assert_eq!(tree.try_sample(&mut rng).unwrap_err(), Error::InsufficientNonZero);
    }
    ...
    fn test_invalid_weight_error() {
        assert_eq!(WeightedTreeIndex::<i32>::new([1, -1]).unwrap_err(), Error::InvalidWeight);
        #[allow(clippy::needless_borrows_for_generic_args)]
