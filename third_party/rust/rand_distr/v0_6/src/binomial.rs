...
impl Binomial {
    ...
    pub fn new(n: u64, p: f64) -> Result<Binomial, Error> {
        ...
        if p == 0.0 {
            return Ok(Binomial { method: Method::Constant(0) });
        }

        if p == 1.0 {
            return Ok(Binomial { method: Method::Constant(n) });
        }
        ...
        let method = if np < BINV_THRESHOLD {
            ...
            if q == 1.0 {
                ...
                let s = p / q;
                Method::Binv(Binv { r: q.powf(n as f64), s, a: (n as f64 + 1.0) * s, n }, flipped)
            }
        } else {
            let q = 1.0 - p;
            let npq = np * q;
...
fn btpe<R: Rng + ?Sized>(btpe: Btpe, flipped: bool, rng: &mut R) -> u64 {
    ...
    loop {
        ...
        let y_sub_m = if y > m { (y - m) as f64 } else { -((m - y) as f64) };
        if alpha
            > x_m * (f1 / x1).ln()
