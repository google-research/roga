...
impl Hypergeometric {
    ...
    pub fn new(
        ...
        let sampling_method = if m - f64::max(0.0, k as f64 - n2 as f64) < HIN_THRESHOLD {
            ...
            SamplingMethod::RejectionAcceptance { m, a, lambda_l, lambda_r, x_l, x_r, p1, p2, p3 }
        };
            ...
            RejectionAcceptance { m, a, lambda_l, lambda_r, x_l, x_r, p1, p2, p3 } => {
                ...
                loop {
                    ...
                    if m < 100.0 || y <= 50.0 {
                        ...
                        let dr = if r < 0.0 { xm * r.powi(4) / (1.0 + r) } else { xm * r.powi(4) };
                        let ds = if s < 0.0 { xn * s.powi(4) / (1.0 + s) } else { xn * s.powi(4) };
                        let dt = if t < 0.0 { xk * t.powi(4) / (1.0 + t) } else { xk * t.powi(4) };
                        let de = if e < 0.0 { nm * e.powi(4) / (1.0 + e) } else { nm * e.powi(4) };
