...
impl<const AB: BlockSize, const Z: BucketSize> PositionMap<AB, Z> {
    pub fn write_position_block<R: Rng + CryptoRng>(
        &mut self,
        address: Address,
...
impl<const AB: BlockSize, const Z: BucketSize> PositionMap<AB, Z> {
    pub fn new<R: CryptoRng + Rng>(
        ...
        log::info!("PositionMap::new(number_of_addresses = {})", number_of_addresses);
...
impl<const AB: BlockSize, const Z: BucketSize> Oram for PositionMap<AB, Z> {
    ...
    fn access<R: Rng + CryptoRng, F: Fn(&TreeIndex) -> TreeIndex>(
        &mut self,
        address: Address,
        callback: F,
        rng: &mut R,
