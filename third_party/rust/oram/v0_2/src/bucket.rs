...
impl<V: OramBlock> PathOramBlock<V> {
    ...
    pub fn dummy() -> Self {
        Self { value: V::default(), address: Self::DUMMY_ADDRESS, position: Self::DUMMY_POSITION }
    }
...
impl<V: ConditionallySelectable> ConditionallySelectable for PathOramBlock<V> {
    fn conditional_select(a: &Self, b: &Self, choice: Choice) -> Self {
        ...
        PathOramBlock::<V> { value, address, position }
    }
}
...
impl<V: OramBlock, const Z: BucketSize> std::fmt::Debug for Bucket<V, Z> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self_is_dummy {
            ...
            f.debug_struct("Bucket").field("blocks", &self.blocks).finish()
        }
    }
}
...
impl<V: OramBlock, const Z: BucketSize> Default for Bucket<V, Z> {
    fn default() -> Self {
        Self { blocks: [PathOramBlock::<V>::dummy(); Z] }
    }
}
