    ...
    Prod<Z, MetaSize>: ArrayLength<u8> + PartialDiv<U8>,
{
    ...
    fn stash_size(&self) -> usize {
        ...
        for idx in 0..self.stash_data.len() {
            ...
            stash_count.cmov(!meta_is_vacant(&self.stash_meta[idx]), &stash_count_incremented);
        }
    ...
    fn access<T, F: FnOnce(&mut A64Bytes<ValueSize>) -> T>(&mut self, key: u64, f: F) -> T {
        ...
        // Visit it and then insert it into the stash.
        {
            ...
            self.branch.ct_find_and_remove(1.into(), &key, &mut data, &mut meta);
    ...
    Prod<Z, MetaSize>: ArrayLength<u8> + PartialDiv<U8>,
{
    ...
    pub fn checkout(
        ...
        debug_assert!(self.leaf == 0);
        self.data.resize_with(leaf.height() as usize + 1, Default::default);
        self.meta.resize_with(leaf.height() as usize + 1, Default::default);
