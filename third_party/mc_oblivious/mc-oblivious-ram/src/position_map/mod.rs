...
impl<R: RngCore + CryptoRng> TrivialPositionMap<R> {
    ...
    pub fn new(size: u64, height: u32, rng_maker: &mut impl FnMut() -> R) -> Self {
        assert!(height < 32, "Can't use u32 position map when height of tree exceeds 31");
        Self { data: vec![0u32; size as usize], height, rng: rng_maker() }
    }
}
...
impl<R: RngCore + CryptoRng> PositionMap for TrivialPositionMap<R> {
    ...
    fn write(&mut self, key: &u64, new_val: &u64) -> u64 {
        ...
        old_val.cmov(old_val.ct_eq(&0), &1u32.random_child_at_height(self.height, &mut self.rng));
        old_val as u64
    }
}
    ...
    > PositionMapCreator<R> for U32PositionMapCreator<ValueSize, R, OC>
{
    fn create<M: 'static + FnMut() -> R>(
        ...
        if size <= 4096 {
            ...
            Box::new(ORAMU32PositionMap::<ValueSize, OC::Output, R>::new::<OC, M>(
                size, height, stash_size, rng_maker,
            ))
        } else {
            panic!("height = {}, but we didn't implement u64 position map yet", height)
        }
    }
}
