...
mod tests {
    ...
    fn destination_leaf_for_bucket_dest(block: i32, zero_index_height: i32, leaf: u64) -> u64 {
        let mask = if block < zero_index_height + 1 { 1 << (zero_index_height - block) } else { 0 };
        mask | leaf
    }
    ...
    fn populate_branch_with_fixed_data(
        ...
        let branch_20 =
            BranchDataConfig { leaf: 20, intended_leaves_for_data_to_insert: vec![24, 27, 18, 23] };
        let branch_16 =
            BranchDataConfig { leaf: 16, intended_leaves_for_data_to_insert: vec![31, 30, 20, 19] };
