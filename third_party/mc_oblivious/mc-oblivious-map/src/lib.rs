    ...
    Sum<KeySize, ValueSize>: ArrayLength<u8> + Sub<KeySize, Output = ValueSize> + PartialDiv<U8>,
{
        ...
        M: 'static + FnMut() -> RngType,
    {
        ...
        debug_assert!(num_buckets & (num_buckets - 1) == 0, "num_buckets must be a power of two");
    ...
    fn vartime_write_extended(
        ...
        while result_code == OMAP_OVERFLOW {
            if eviction_retries > 0 {
                ...
                // through this loop.
                let oram = if next_oram == 0 { &mut self.oram1 } else { &mut self.oram2 };
                ...
                while !eviction_indices.is_empty() {
                    ...
                    // but the access patterns are completely predicatable based on number of passes
                    // through this loop.
                    let oram = if next_oram == 0 { &mut self.oram2 } else { &mut self.oram1 };
...
mod testing {
    ...
    fn sanity_check_omap_z4_4() {
        run_with_several_seeds(|rng| {
            ...
            assert_eq!(OMAP_NOT_FOUND, omap.vartime_write(&a8_8(1), &a8_8(2), 0.into()));
            ...
            assert_eq!(&temp, &a8_8(3), "omap.read must not modify the output on not_found");
            assert_eq!(OMAP_NOT_FOUND, omap.vartime_write(&a8_8(2), &a8_8(20), 1.into()));
            assert_eq!(OMAP_FOUND, omap.read(&a8_8(2), &mut temp));
            assert_eq!(&temp, &a8_8(20));
            assert_eq!(OMAP_FOUND, omap.vartime_write(&a8_8(2), &a8_8(30), 0.into()));
            assert_eq!(OMAP_FOUND, omap.read(&a8_8(2), &mut temp));
            assert_eq!(&temp, &a8_8(20), "omap.write must not modify when overwrite is disallowed");
            assert_eq!(OMAP_FOUND, omap.vartime_write(&a8_8(2), &a8_8(30), 1.into()));
            assert_eq!(OMAP_FOUND, omap.read(&a8_8(2), &mut temp));
    ...
    fn sanity_check_omap_z4_2097152() {
        run_with_several_seeds(|rng| {
            ...
            assert_eq!(OMAP_NOT_FOUND, omap.read(&a8_8(1), &mut temp));
            assert_eq!(OMAP_NOT_FOUND, omap.vartime_write(&a8_8(1), &a8_8(2), 1.into()));
            assert_eq!(OMAP_FOUND, omap.read(&a8_8(1), &mut temp));
            assert_eq!(&temp, &a8_8(2));
            assert_eq!(OMAP_FOUND, omap.vartime_write(&a8_8(1), &a8_8(3), 1.into()));
            assert_eq!(OMAP_FOUND, omap.read(&a8_8(1), &mut temp));
